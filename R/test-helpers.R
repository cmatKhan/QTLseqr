...
bsa_file_fixtures_extended <- function(env = parent.frame()){
  ...
  gatk_table <- data.frame(
    ...
    SLB0025.GQ = c(99, 10, 6, 99, 99),
    SLB0025.GT = c("G", "C", "C", "A", "T"),
    SLB0031.AD = rep("200,100", 5),
    SLB0031.DP = seq(100,500,100),
    SLB0031.PL = c("0,6352", "0,6879", "0,6604", "0,874", "0,620"),
    SLB0031.GQ = c(99, 10, 6, 99, 99),
    SLB0031.GT = c("G", "C", "C", "A", "T"),
    SLB0035.AD = rep("200,100", 5),
    SLB0035.DP = seq(100,500,100),
    SLB0035.PL = c("0,2666", "0,4700", "0,4721", "0,677", "0,1396"),
    SLB0035.GQ = c(99, 10, 6, 99, 99),
    SLB0035.GT = c("G", "C", "C", "A", "T"),
    check.names = FALSE
  )
  # Save the GATK table to a temporary file
  write.table(gatk_table, gatk_table_path, sep = "\t", row.names = FALSE)
  ...
  meta_df = data.frame(
    sample = c("SLB0021", "SLB0025", "SLB0031", "SLB0035"),
    group = c("ir7all", "ir7all", "ir7all", "ir7all"),
    pool = c(1, 2, 1, 2),
    replicate = c(1, 1, 1, 1),
    condition = c("inoculum", "lung", "inoculum", "lung"),
    sac_day = c(0, 9, 0, 9),
    culture_time = c(0, 0, 0, 0)
  )
...
bsae_obj_fixture = function(add_comparisons = FALSE, extended=FALSE){

  if(extended){
    paths = bsa_file_fixtures_extended()
  } else {
  ...
  if(add_comparisons){
    comparisons(bsae) <- DataFrame(population_1 = 'SLB0021', population_2 = 'SLB0025')
  }
